...
*   **✅ Validate an EPUB (`check`)**: Check any EPUB against the spec — `epublift check book.epub` — with epubcheck-compatible message IDs (`RSC-005`, `OPF-053`, …). Powered by [**epubveri**](https://github.com/veripublica/epubveri), our own **pure-Rust, JVM-free** alternative to W3C's `epubcheck` (no Java, no external process), behind the opt-in `validate` build feature. Grep-style exit code for CI; `--json` for tooling. Pre-1.0 (99.7% message-ID recall on epubcheck's own test suite — not yet full parity). See the [Validation guide](docs/validate.md).
