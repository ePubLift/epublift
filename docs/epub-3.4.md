...
| **HTML syntax removed → XHTML-only** | ✅ **Guard (lint).** Our pipeline is string/regex-based, so HTML-syntax content passes through unchanged — which would be non-conformant in 3.4. For a `--target 3.4` run we **detect** content documents still in HTML syntax (the reliable signal: an HTML void element that isn't self-closed, e.g. bare `<br>`/`<img>`) and **report** them; we don't auto-convert HTML→XHTML (out of scope, needs a full HTML5 parser). 3.3 allows HTML syntax, so this is 3.4-gated. | No change. | Done (lint; `util.rs` + report). |
